...
const AuthModal: React.FC<AuthModalProps> = ({ onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[300] bg-emerald-950/60 backdrop-blur-xl flex items-center justify-center p-4">
      <div className="max-w-md w-full bg-white rounded-[3rem] p-10 shadow-2xl space-y-8 animate-lux-in relative">
        ...
        {error && (
          <div className="p-4 bg-red-50 text-red-600 text-[10px] font-bold uppercase rounded-2xl text-center">
            {typeof error === 'string' && error.toLowerCase().includes('unauthorized-domain') ? (
              <div className="space-y-2">
                <p>Domain Unauthorized</p>
                <p className="text-[8px] normal-case opacity-70">
                  Please add <b>{window.location.hostname}</b> to 'Authorized Domains' in your Firebase Console (Authentication > Settings > Authorized domains).
                </p>
              </div>
            ) : error}
          </div>
        ...
        <div className="space-y-4">
          <button 
            ...
            disabled={loading}
            className="w-full py-4 border border-emerald-50 rounded-2xl flex items-center justify-center gap-4 hover:bg-emerald-50 transition-all group"
          >
            ...
            <span className="text-[10px] font-bold text-emerald-950 uppercase tracking-widest">Continue with Google</span>
          </button>

          <div className="flex items-center gap-4">
            <div className="flex-1 h-[1px] bg-emerald-50"></div>
            <span className="text-[8px] font-bold text-emerald-950/20 uppercase tracking-widest">or email</span>
            <div className="flex-1 h-[1px] bg-emerald-50"></div>
          </div>
        </div>
        ...
        <div className="text-center mt-6">
          <button onClick={() => { setIsLogin(!isLogin); setError(null); }} className="text-[10px] font-bold text-emerald-950/40 uppercase tracking-widest hover:text-gold transition-colors">
