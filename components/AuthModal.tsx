...
const AuthModal: React.FC<AuthModalProps> = ({ onClose }) => {
  ...
  const handleBypass = () => {
    loginAsGuest();
    onClose(); // Auto-close upon guest entry
  };

  return (
    <div className="fixed inset-0 z-[300] bg-emerald-950/90 backdrop-blur-2xl flex items-center justify-center p-4">
      <div className="max-w-md w-full bg-white rounded-[3.5rem] p-12 shadow-2xl space-y-10 animate-lux-in relative border border-emerald-50">
        <div className="text-center space-y-3">
          <div className="w-12 h-12 bg-emerald-50 rounded-full mx-auto flex items-center justify-center text-gold border border-emerald-100/50 mb-4">
            <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 11c0 3.517-1.009 6.799-2.753 9.571m-3.44-2.04l.054-.09A10.003 10.003 0 0012 20c4.083 0 7.774-2.436 9.262-6H12z" /></svg>
          </div>
          <h2 className="text-3xl font-serif text-emerald-950 italic">{isLogin ? 'Maison Identity' : 'Registry Entry'}</h2>
          <p className="text-[10px] text-emerald-950/30 font-bold uppercase tracking-[0.5em]">Neural Visual Intelligence Access</p>
        </div>

        {error && (
          <div className={`p-8 border rounded-[2.5rem] space-y-6 animate-in zoom-in-95 duration-500 ${isUnauthorizedDomain ? 'bg-amber-50/50 border-amber-200 shadow-xl shadow-amber-900/5' : 'bg-red-50 border-red-100'}`}>
            {isUnauthorizedDomain ? (
              <div className="space-y-6 text-center">
                <div className="flex flex-col items-center gap-4 text-amber-800">
                  ...
                  <p className="text-[11px] font-bold uppercase tracking-[0.3em]">Environment Intervention</p>
                </div>
                <div className="space-y-4">
                  <p className="text-[12px] text-amber-900/70 leading-relaxed font-serif italic px-2">
                    The domain <code className="bg-white/80 px-2 py-0.5 rounded text-amber-950 border border-amber-200 font-mono text-[10px]">{currentHostname}</code> is currently restricted for cloud synchronization.
                  </p>
                  <div className="pt-4 border-t border-amber-200/50">
                    ...
                    <p className="text-[9px] mt-5 text-amber-800/50 font-bold uppercase tracking-widest italic">Experience full neural power locally.</p>
                  </div>
                </div>
              </div>
            ) : (
              <p className="text-[10px] font-bold uppercase tracking-widest text-red-600 text-center leading-relaxed">{errorString}</p>
            )}
          </div>
        )}
        ...
        {!isUnauthorizedDomain && (
          <>
            ...
            <div className="text-center pt-4">
              <button onClick={() => { setIsLogin(!isLogin); setError(null); }} className="text-[10px] font-bold text-emerald-950/30 uppercase tracking-[0.3em] hover:text-gold transition-colors">
